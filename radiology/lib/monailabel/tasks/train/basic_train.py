...
class BasicTrainTask(TrainTask):
    ...
    def partition_datalist(self, context: Context, shuffle=False):
        dataset_mode = context.request.get("dataset_mode", "split")
        
        if dataset_mode == "paths":
            # Load from explicit paths
            train_datalist = self._load_external_ds(context.request.get("train_path"))
            val_datalist = self._load_external_ds(context.request.get("val_path"))
            
            if not train_datalist:
                raise ValueError("train_path must be provided when dataset_mode='paths'")
        else:
